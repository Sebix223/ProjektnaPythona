        ...
        try:
            ...
            if ID is not None:
                if ID not in df['ID'].values:
                    raise ValueError(f"ksiązka o ID {ID} nie istnieje w bazie danych.Spróboj ponownie")
                df = df[df['ID'] != ID]
            elif title is not None:
                if title not in df['TITLE'].values:
                    raise ValueError(f"Książka o tytule {title} nie istnieje w bazie danych. Spróboj ponownie")
